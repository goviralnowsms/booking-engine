...
class TourPlanApiClient {
  constructor() {
    this.apiUrl = TOURPLAN_CONFIG.API_URL || process.env.TOURPLAN_API_URL
    this.agentId = TOURPLAN_CONFIG.AGENT_ID || process.env.TOURPLAN_AGENT_ID
    this.password = TOURPLAN_CONFIG.PASSWORD || process.env.TOURPLAN_PASSWORD
  ...
  async searchTours(searchParams) {
    try {
      // Create search XML request using the main API instance
      const xmlRequest = this.createOptionInfoRequest(
   ...
   */
  createOptionInfoRequest(opt, info, dateFrom, dateTo, roomConfigs) {
...
<HostConnectRequest>
  ...
  async makeRequest(endpoint, xmlData) {
    try {
      if (!this.apiUrl) {
        throw new Error("Tourplan API URL not configured")
      }
      ...
      console.error("Tourplan client request failed:", error)
      throw error
    }
  }
}
...
let tourplanClient = null

function getTourplanAPI() {
  if (!tourplanClient) {
    tourplanClient = new TourPlanApiClient()
  }
  return tourplanClient
}
