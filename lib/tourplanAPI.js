...
// In-memory mock storage for browser compatibility
const mockResponses = new Map();

// Helper function to save mock responses (in-memory for browser compatibility)
function saveMockResponse(requestType, requestData, responseData) {
  if (!USE_MOCKS) return;
  
  try {
    mockResponses.set(requestType, { request: requestData, response: responseData });
    console.log(`Mock response saved for ${requestType}`);
  } catch (err) {
    console.error('Error saving mock response:', err);
  }
}

// Helper function to get mock responses (from memory for browser compatibility)
function getMockResponse(requestType) {
  try {
    if (mockResponses.has(requestType)) {
      const mockData = mockResponses.get(requestType);
      console.log(`Using mock response for ${requestType}`);
...
if (USE_MOCKS && !mockResponses.has('GetServiceButtonDetails')) {
