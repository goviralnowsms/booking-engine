/**
 * NOTE: Any network failure (VPN down, DNS, IP not whitelisted, etc.)
 * is caught here so that the UI never crashes. Upper-level functions
 * (`searchTours`, `getOptionInfo`, …) will detect the empty object and
 * seamlessly fall back to mock data.
 */
...
export class TourplanAPI {
  ...
  private async makeRequest(xmlBody: string, useSoapEndpoint = false): Promise<any> {
    const url = useSoapEndpoint ? this.baseUrl : this.baseUrl
    ...
    try {
      const response = await fetch(url, {
        ...
        body: xmlBody,
        // 15-second hard timeout so the UI does not hang forever
        next: { revalidate: 0, tags: ["tourplan"] },
      })
      ...
      console.error("⚠️  Tourplan API network failure:", err)
      console.warn("🔄 Falling back to local mock data\n")

      // Instead of propagating the error, return an empty object
      // Caller functions already know how to switch to mock data
      return {}
    }
  }
